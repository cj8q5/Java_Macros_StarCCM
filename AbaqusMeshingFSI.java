...
public class AbaqusMeshingFSI extends StarMacro 
{
	public void execute() 
	{
		...
		boolean ssOrFSI = Boolean.parseBoolean(reader.getStringData("steadyStateOrFSI"));
		...
		String fileLocation = "D:/Users/cj8q5/Simulations/Current Simulation/Abaqus/";
		String abqFileName = "StarCCM+_FluidInput_" + (int)(plateHeight/0.0254*1000) + 
		...
		abaqus.setAbaqusExecutionSettings(
				couplingScheme + "_FSI_" + (int)Math.abs(initialVel[1]) + "_Abaqus_" + 
		...
		{		
			double x_origin = r_in*Math.cos(Math.PI/4);
			double z_origin = r_in*Math.sin(Math.PI/4);
			centerPlane.createSectionPlane(new double[] {1.0, 0, -1.0}, new double[] {x_origin, 0, z_origin}, "CenterPlane");
		}
		...
		{		
			...
			smChLinePart = smChLineProbe.createLineProbe(lgChLineProbeCoord_0, lgChLineProbeCoord_1, 1000, "LargeChannelLineProbe");
			...
			lgChLinePart = lgChLineProbe.createLineProbe(smChLineProbeCoord_0, smChLineProbeCoord_1, 1000, "SmallChannelLineProbe");
		}
		else if(plateGeometry.equals("curved"))
		{		
			double[] lgChLineProbeCoord_0 = {0.0, -outletLength, r_in - (largeChannelHeight*0.5)};
			double[] lgChLineProbeCoord_1 = {0.0, plateLength + inletLength, r_in - (largeChannelHeight*0.5)};
			smChLinePart = smChLineProbe.createLineProbe(lgChLineProbeCoord_0, lgChLineProbeCoord_1, 1000, "LargeChannelLineProbe");
			
			double[] smChLineProbeCoord_0 = {0.0, -outletLength, r_in + smallChannelHeight*0.5 + plateHeight};
			double[] smChLineProbeCoord_1 = {0.0, plateLength + inletLength, r_in + smallChannelHeight*0.5 + plateHeight};
			lgChLinePart = lgChLineProbe.createLineProbe(smChLineProbeCoord_0, smChLineProbeCoord_1, 1000, "SmallChannelLineProbe");
		}
		...
		wallYplusPlot.addObjects2XYPlot(wallYplus_XYPlot, "Fluid", 
				new String[] {"Fluid.FSI_Back", "Fluid.FSI_Front", "Fluid.FSI_Bottom", "Fluid.FSI_Top"});
		...
		tools.createXYZInternalTable("Fluid", new String[] {"FSI_Back", "FSI_Front"}, "FSI_Back_Pressure_Table");
		...
		tools.createXYZInternalTable("Fluid", new String[] {"FSI_Back", "FSI_Front"}, "FSI_Front_Pressure_Table");
		tools.setXYZInternalTableFieldFunction("StaticPressure");
		//tools.extractAndExportXYZInternalTableData(currentDirectory + File.separator + "XYZ_Pressure.csv");
		...
		String saveLocation = currentDirectory + File.separator + 
			couplingScheme + "_FSI_" + (int)Math.abs(initialVel[1]) + "_StarCCM_" + 
				(int)(plateHeight/0.0254*1000) + "_" + (int)(smallChannelHeight/0.0254*1000) + "_" + (int)(largeChannelHeight/0.0254*1000) +".sim";
